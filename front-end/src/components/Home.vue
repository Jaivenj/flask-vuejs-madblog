  ...
  <div>
    <div style="padding:10px">
      ...
      <Post v-for="(post, index) in posts.items" v-bind:key="index" :post="post" v-on:delete-post="onDeletePost(post)"
        v-on:edit-post="onEditPost(post)"></Post>
    </div>
    ...
    <div style="margin-bottom:80px">
      <quill-editor v-model="PostForm.Body" ref="myQuillEditor" :options="editorOption" @blur="onEditorBlur($event)"
        @focus="onEditorFocus($event)" @change="onEditorChange($event)" style="padding:10px;height:200px">
      </quill-editor>
    </div>

    <div >
      <el-button style="float: right; margin:0px 20px 10px" @click="Cancle_post()" 主要按钮>Cancle</el-button>
      <el-button style="float: right;margin-bottom:10px   " @click="Sumbit_post()" 主要按钮>Sumbit</el-button>
    </div>
