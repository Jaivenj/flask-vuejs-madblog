<template>
  <div>
    <el-dialog title="编辑文章" :show-close=false :visible="dialogFormVisible" v-loading="loading" :modal="false"
>
      <el-form :model="editForm">
        ...
        <el-form-item label="Body" :label-width="formLabelWidth">
          <quill-editor ref="myQuillEditor" v-model="editForm.body">
          </quill-editor>
        </el-form-item>
      </el-form>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogFormVisible = false, Update_form(post)">确 定</el-button>
      </div>
    </el-dialog>
    ...
    <div style="padding:10px">
      ...
      <!-- 阅览博客列表 -->
      <Post v-for="(post, index) in posts.items" v-bind:key="index" :post="post" v-on:delete-post="onDeletePost(post)"
        v-on:edit-post="onEditPost(post)">
      </Post>
    </div>

    <el-pagination background layout="prev, pager, next" :total="posts._meta.total_items "
      :page-size="posts._meta.per_page" v-on:page-count="posts._meta.total_pages" style="padding:10px;text-align: right"
      @prev-click="Prev_click()" @next-click="Next_click()" @current-change="Current_Change()"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onDeletePost(post) {
        this.$confirm(`此操作将永久删除文章《${post.title}》, 是否继续?`, '提示', {
